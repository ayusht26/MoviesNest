...
export default function TopTenCard({ rank, id, title, poster_path, vote_average, media_type = 'movie', release_date, first_air_date }: Props) {
  ...
  return (
    <motion.a
      href={href}
      className="group block relative flex-shrink-0 w-[150px] sm:w-[170px] lg:w-[190px] rounded-lg overflow-hidden glass-card cursor-pointer shadow-level-2 hover:shadow-level-4 transition-shadow duration-300"
    ...
    >
      ...
      <div className="absolute top-2 left-2 z-20 flex items-center justify-center px-2 py-0.5 rounded bg-cyan text-black font-mono text-[9px] font-bold tracking-wider uppercase shadow-sm animate-pulse-glow">
        TOP {formattedRank}
      </div>
      ...
      <div className="relative aspect-[2/3] overflow-hidden bg-canvas-soft-2">
        ...
        {/* Hover play overlay */}
        <motion.div
          className="absolute inset-0 bg-[#171717]/40 flex flex-col items-center justify-center p-3 z-10"
          initial={{ opacity: 0 }}
        ...
        {/* Rating badge */}
        <div className="absolute top-2 right-2 z-20 flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-black/60 backdrop-blur-sm text-[10px] text-yellow-400 font-mono font-bold">
      ...
      {/* Focus hairline ring */}
      <motion.div
        className="absolute inset-0 rounded-lg pointer-events-none border border-transparent z-20"
